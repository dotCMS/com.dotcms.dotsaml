...
import com.dotcms.saml.config.EndpointHelper;
...
/**
 * Access filter for SAML plugin, it does the autologin and also redirect to the IDP if the user is
 * not logged in.
 * 
 * @author jsanca
 */
public class SamlAccessFilter extends SamlFilter implements Filter {
    ...
    @VisibleForTesting
    public SamlAccessFilter(final SamlAuthenticationService samlAuthenticationService,
                    final MetaDataXMLPrinter metaDataXMLPrinter, final HostWebAPI hostWebAPI, final LanguageWebAPI languageWebAPI,
                    final PermissionAPI permissionAPI, final IdentifierAPI identifierAPI, final ContentletAPI contentletAPI,
                    final UserWebAPI userWebAPI, final LoginServiceAPI loginService) {
    ...
    public void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse, final FilterChain chain)
        ...
        try {
            ...
            if (idpConfig != null && idpConfig.isEnabled()) {
                ...
                if (!super.checkAccessFilters(httpServletRequest.getRequestURI(), EndpointHelper.getAccessFilterArray(idpConfig))
                    ...
                    final AutoLoginResult autoLoginResult =
                                    super.autoLogin(httpServletRequest, httpServletResponse, session, idpConfig);
                    ...
                    if (null == session || super.isNotLogged(httpServletRequest, session)) {
                        ...
                        final String originalRequest =
                                        httpServletRequest.getRequestURI() + ((null != httpServletRequest.getQueryString())
                                                        ? "?" + httpServletRequest.getQueryString()
                                                        : StringUtils.EMPTY);

                        redirectAfterLogin = (UtilMethods.isSet(httpServletRequest.getParameter(REFERRER_PARAMETER_KEY)))
                        ...
                        Logger.debug(this.getClass(), "Executing SAML Login Redirection with request: " + redirectAfterLogin);
                        ...
                        try {
                            ...
                            Logger.error(this,
                                            "An error occurred when redirecting to the IdP Login page: " + exception.getMessage(),
                                            exception);
