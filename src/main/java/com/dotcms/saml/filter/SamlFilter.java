package com.dotcms.saml.filter;
...
import com.dotcms.saml.config.IdpConfig;
import com.dotcms.saml.config.MetaDataHelper;
import com.dotcms.saml.init.Initializer;
import com.dotcms.saml.init.SamlInitializer;
import com.dotcms.saml.key.DotSamlConstants;
import com.dotcms.saml.service.OpenSamlAuthenticationServiceImpl;
import com.dotcms.saml.service.SamlAuthenticationService;
import com.dotcms.saml.util.InstanceUtil;
import com.dotcms.saml.util.MetaDataXMLPrinter;
import com.dotcms.saml.util.SamlUtils;
...
public class SamlFilter implements Filter {
    ...
    public SamlFilter() {
        this(InstanceUtil.newInstance(Config.getStringProperty(DotSamlConstants.DOT_SAML_AUTHENTICATION_SERVICE_CLASS_NAME, null),
    ...
    @VisibleForTesting
    public SamlFilter(final SamlAuthenticationService samlAuthenticationService, final MetaDataXMLPrinter metaDataXMLPrinter,
                    final HostWebAPI hostWebAPI, final LanguageWebAPI languageWebAPI, final PermissionAPI permissionAPI,
                    final IdentifierAPI identifierAPI, final ContentletAPI contentletAPI, final UserWebAPI userWebAPI,
    ...
    /**
     * This method checks if some path does not wants to be treatment by the {@link SamlFilter} An
     * example of exception might be the destroy.jsp, so on.
     * 
    ...
    /**
     * Determine if the path is Backend Admin, usually it is for /c && /admin or if the path is a file
     * or path, will check if the user has permission
     * 
     * @param uri {@link String}
    ...
    public boolean doLogout(final HttpServletResponse response, final HttpServletRequest request, final HttpSession session,
                    final IdpConfig idpConfig) throws IOException, ServletException {
        ...
        try {
            if (null != nameID && null != samlSessionIndex) {
                Logger.debug(this, "The URI '" + request.getRequestURI()
                                + "' is a logout request. Executing the logout call to SAML");
    ...
    protected void doAuthenticationLoginSecurityLog(final HttpServletRequest request, final IdpConfig idpConfig,
        ...
        try {
            final Host host = this.hostWebAPI.getCurrentHost(request);
            final String env = this.isFrontEndLoginPage(request.getRequestURI()) ? "frontend" : "backend";
            final String log = new Date() + ": Successfull SAML login for Site '" + host.getHostname() + "' with IdP " + "ID: "
                            + idpConfig.getId() + " (" + env + ") from " + request.getRemoteAddr() + " for user: "
    ...
    public boolean printMetaData(final HttpServletRequest request, final HttpServletResponse response, final IdpConfig idpConfig)
                    throws ServletException {
        // First, get the Entity descriptor.
        final EntityDescriptor descriptor =
                        MetaDataHelper.getMetaDescriptorService(idpConfig).getServiceProviderEntityDescriptor(idpConfig);
        Writer writer = null;
        boolean isOK;
    ...
    protected AutoLoginResult autoLogin(final HttpServletRequest request, final HttpServletResponse response,
        ...
        if (null != user) {
            ...
            // create it!
            try {
                Logger.debug(this, "User with ID '" + user.getUserId() + "' has been returned by SAML Service. User " + "Map: "
                                + user.toMap());
            } catch (Exception e) {
                Logger.error(this,
                                "An error occurred when retrieving data from user '" + user.getUserId() + "': " + e.getMessage(),
                                e);
            }

            final boolean doCookieLogin = this.loginService.doCookieLogin(PublicEncryptionFactory.encryptString(user.getUserId()),
                            request, response);
     ...
     *
     * @return If the user or its URI can be associated to the dotCMS back-end login, returns
    ...
    /**
     * Analyzes the incoming URI and determines whether it belongs to dotCMS back-end login or logout
     * URIs or not.
     *
     * @param uri The incoming URI.
     *
     * @return If the URI can be associated to the dotCMS back-end login or logout, returns
     *         {@code true}. Otherwise, returns {@code false}.
     */
